...
public class AidZhantkinMyersInverse extends Thread {
	...
	public AidZhantkinMyersInverse(int[][] inputTree, int inputSize, int inputTreeSize, double mL, double ls) {
        this.tree = inputTree;
        this.size = inputSize;
	...
	public AidZhantkinMyersInverse(int[][] inputTree, int inputSize, int inputTreeSize, double mL, double ls, int[] memberTranslation) {
        this.tree = inputTree;
        this.size = inputSize;
        this.treeSize = inputTreeSize;
        this.lambdaLimit = mL;
	...
	public void analyse() {
        ...
        for (double lambda_counter = 1.0; lambda_counter <= this.lambdaLimit; lambda_counter += this.lambdaStep) {
        	
        	double lambda = 1.0 / lambda_counter;
            ...
            for (int row = 0; row < this.treeSize; row++) {
                ...
                for (int i = 1; i < this.treeSize; i++) {
                    part2 += Math.log((double) hit_matrix_B[i] + (double) this.matrix3[row][i] * (lambda - 1.0));
                }
...
//        System.out.println("hit_matrix_B[0] = " + hit_matrix_B[0] + "Value1 = " + maxScoreSelection + " Value2 = " + value2[maxScoreRow] + "BigLambda = " + (2.0 * (maxScoreSelection - maxScoreNoSelection)));
        ...
        for (int i = 0; i < this.size - 1; i++){
        	...
        	this.lambdaResults[i] = 1.0 / (1.0 + (this.lambdaStep * (double)index));
        }
    }
